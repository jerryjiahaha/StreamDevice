...
class FilterInterface : StreamBusInterface
#ifndef EPICS_3_13
 , epicsTimerNotify
#endif
{
...
RegisterStreamBusInterface(FilterInterface);

FilterInterface::
FilterInterface(Client* client) : StreamBusInterface(client)
{
    pasynCommon = NULL;
    pasynOctet = NULL;
    intrPvtOctet = NULL;
...
StreamBusInterface* FilterInterface::
    ...
    const char* busname, int addr, const char*)
{
    ...
    {
        debug ("FilterInterface::getBusInterface(%s, %d): "
...
lockRequest(unsigned long lockTimeout_ms)
{
    ...
    debug("FilterInterface::lockRequest(%s, %ld msec)\n",
...
connectToAsynPort()
{
    ...
    debug("FilterInterface::connectToAsynPort(%s)\n",
        clientName());
    status = pasynManager->isConnected(pasynUser, &connected);
    if (status != asynSuccess)
    {
...
*/
    ...
    debug("FilterInterface::connectToAsynPort(%s) is %s connected\n",
...
{
    int connected;
    debug("FilterInterface::lockHandler(%s)\n",
        clientName());
...
unlock()
{
    debug("FilterInterface::unlock(%s)\n",
        clientName());
...
{
#ifndef NO_TEMPORARY
    debug("FilterInterface::writeRequest(%s, \"%s\", %ld msec)\n",
...
writeHandler()
{
    debug("FilterInterface::writeHandler(%s)\n",
        clientName());
    asynStatus status;
    size_t written = 0;
...
{
    debug("FilterInterface::readRequest(%s, %ld msec reply, "
    ...
    debug("FilterInterface::readRequest %s: "
    ...
    char *data, size_t numchars, int eomReason)
{
    FilterInterface* interface =
        static_cast<FilterInterface*>(pasynUser->userPvt);
...
asynReadHandler(const char *buffer, size_t received, int eomReason)
{
...
#ifndef NO_TEMPORARY
    debug("FilterInterface::asynReadHandler(%s, buffer=\"%s\", "
    ...
    debug("FilterInterface::asynReadHandler(%s) readMore=%li, ioAction=%s \n",
...
void intrCallbackInt32(void* /*pvt*/, asynUser *pasynUser, epicsInt32 data)
{
    FilterInterface* interface =
        static_cast<FilterInterface*>(pasynUser->userPvt);
    debug("FilterInterface::intrCallbackInt32 (%s, %ld)\n",
    ...
    epicsUInt32 data)
{
    FilterInterface* interface =
        static_cast<FilterInterface*>(pasynUser->userPvt);
    debug("FilterInterface::intrCallbackUInt32 (%s, %ld)\n",
        interface->clientName(), (long int) data);
    if (interface->eventMask)
    {
        if (data & interface->eventMask)
    ...
    switch (ioAction)
    {
        ...
        case AsyncRead:
            ...
            else
            {
                ...
                debug("FilterInterface::timerExpired %s: "
...
{
    FilterInterface* interface =
        static_cast<FilterInterface*>(pcallback->user);
    interface->timerExpired();
}
#else
epicsTimerNotify::expireStatus FilterInterface::
...
connectRequest(unsigned long connecttimeout_ms)
{
    ...
    debug("FilterInterface::connectRequest %s\n",
        clientName());
    status = pasynManager->queueRequest(pasynUser,
...
disconnectRequest()
{
    ...
    debug("FilterInterface::disconnectRequest %s\n",
        clientName());
    status = pasynManager->queueRequest(pasynUser,
...
disconnectHandler()
{
    ...
    pasynManager->isConnected(pasynUser, &connected);
    debug("FilterInterface::disconnectHandler %s is %s disconnected\n",
        clientName(), !connected ? "already" : "not yet");
    if (connected)
    {
...
finish()
{
    debug("FilterInterface::finish(%s) start\n",
        clientName());
    cancelTimer();
    ioAction = None;
//     if (pasynGpib)
...
//     }
    debug("FilterInterface::finish(%s) done\n",
        clientName());
}
...
void handleRequest(asynUser* pasynUser)
{
    ...
    debug("FilterInterface::handleRequest(%s) %s\n",
...
void handleTimeout(asynUser* pasynUser)
{
    FilterInterface* interface =
        static_cast<FilterInterface*>(pasynUser->userPvt);
    debug("FilterInterface::handleTimeout(%s)\n",
    ...
    switch (interface->ioAction)
    {
        ...
        case Connect:
            ...
            break;
        case Disconnect:
            error("FilterInterface %s: disconnect timeout\n",
                interface->clientName());
